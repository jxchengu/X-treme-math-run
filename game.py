...
if player1 and player2 and (num_players < 3 or player3) and (num_players < 4 or player4):
    ...
    if num_players >=1:
        ...
        with col1:
            number1 = st.number_input(f"Score for {player1}", key="num1, ", min_value=0.0, format="%.2f")
            st.write("Box 1 current number:", number1)

        with col2:
            number2 = st.number_input(f"Score for {player2}", key="num2", min_value=0.0, format="%.2f")
    ...
    if num_players >=3:
        ...
        with col1:
                number3 = st.number_input(f"score for {player3}", key="num3", min_value=0.0, format="%.2f")
                st.write("Box 1 current number:", number3)
    if num_players == 4:
        with col2:
            number4 = st.number_input(f"score for {player4}", key="num4", min_value=0.0, format="%.2f")
